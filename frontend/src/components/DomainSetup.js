...
const DomainSetup = ({clientId, onDomainVerified}) => {
    ...
    const handleDomainSubmit = async () => {
        ...
        try {
            ...
            } else if (setupResponse && setupResponse.message === "Client already has this domain configured") {
                setVerificationMessage("Client already has this domain configured, please select prospect.");
                ...
                <>
                    ...
                    <ul>
                        {dnsRecords.map((record, index) => (
                            <li key={index}>
                                Type: {record.record_type}, Host: {record.host}, Points to: {record.points_to},
                                TTL: {record.ttl}
                            </li>
                        ))}
                    </ul>
